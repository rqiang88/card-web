...
import type { RechargeFormData, Member, Package } from "@/types"
  ...
  const handleSubmit = async (data: RechargeFormData) => {
    try {
      ...
      if (data.type === "package" && selectedPackage) {
        ...
        const priceToUse = selectedPackage.salePrice || selectedPackage.price || 0
      ...
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <Select onValueChange={field.onChange} defaultValue={field.value}>
                  ...
                  <SelectContent>
                    {PAYMENT_METHODS.filter(method => method.value !== 'balance').map((method) => (
                    ...
                    <div className="flex items-center space-x-3 p-3 border rounded-md bg-gray-50">
                      <div className="w-8 h-8 bg-primary/10 rounded-full flex items-center justify-center">
                        <span className="text-sm font-medium text-primary">
                          {selectedMember?.name?.charAt(0) || initialMember?.name?.charAt(0) || '?'}
                        </span>
                      </div>
                      <div className="flex-1">
                        <p className="font-medium text-gray-900">
                          {selectedMember?.name || initialMember?.name || '加载中...'}
                        </p>
              ...
              <FormItem>
                ...
                <FormControl>
                  <Input
                    ...
                    disabled={watchType === "package" && !!selectedPackage}
                  />
                </FormControl>
                <FormMessage />
              </FormItem>
        ...
        <div className="flex justify-end space-x-4 pt-4">
          ...
          <Button type="submit" disabled={loading}>
            {loading ? "提交中..." : initialData ? "更新充值记录" : "创建充值记录"}
          </Button>
        </div>
      </form>
    </Form>
