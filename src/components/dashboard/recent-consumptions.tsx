...
import type { RecentConsumption } from '@/lib/api/dashboard'

export function RecentConsumptions() {
  const [consumptions, setConsumptions] = useState<RecentConsumption[]>([])
  ...
  useEffect(() => {
    const fetchRecentConsumptions = async () => {
      try {
        setLoading(true)
        const data = await dashboardApi.getRecentConsumptions()
        setConsumptions(data)
      } catch (error) {
        console.error('获取最近消费失败:', error)
      } finally {
        setLoading(false)
      }
    }
  ...
  return (
    <Card className="h-full">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <button
          onClick={() => router.push('/dashboard/consumptions')}
        ...
        <div className="space-y-3">
          {consumptions.length === 0 ? (
            ...
            consumptions.map((consumption) => {
              const genderInfo = getGenderDisplay(consumption.customerGender)
              return (
                <div
                  key={consumption.id}
                  onClick={() =>
                    router.push(`/dashboard/consumptions/${consumption.id}`)
                  }
                  ...
                  <div className="relative">
                    <div className="w-10 h-10 bg-gradient-to-br from-orange-500 to-red-600 rounded-full flex items-center justify-center">
                      <span className="text-sm font-medium text-white">
                        {consumption.customerName?.charAt(0) || '?'}
                      </span>
                    </div>
                    <div
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      <p className="text-sm font-medium text-foreground truncate">
                        {consumption.customerName || '未知客户'}
                      </p>
                      <div className="flex items-center text-xs text-muted-foreground">
                        <Calendar className="h-3 w-3 mr-1" />
                        {formatDate(consumption.consumptionTime)}
                      </div>
                    </div>
                    <div className="flex items-center justify-between mt-1">
                      <div className="flex items-center text-xs text-muted-foreground">
                        ...
                        <span className="truncate">
                          {consumption.packageName || '服务消费'}
                        </span>
                      </div>
                      <div className="flex items-center text-sm font-medium text-orange-600">
                        <TrendingDown className="h-3 w-3 mr-1" />
                        {consumption.packageType === 'normal' 
                          ? (consumption.amount && consumption.amount > 0 
