...
import { Plus, Search, Trash2, CreditCard, Clock, User, Package, Phone, Calendar, Coins } from "lucide-react"
...
export default function ConsumptionPage() {
  ...
  const filteredConsumptions = consumptions.filter((consumption: any) =>
    consumption.memberName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  // 计算总金额
  const totalAmount = filteredConsumptions.reduce((sum: number, c: any) => sum + c.amount, 0)
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">
                  ¥{filteredConsumptions.length > 0 ? (totalAmount / filteredConsumptions.length).toFixed(2) : '0.00'}
                </p>
        ...
        <CardContent>
          ...
          <div className="space-y-4">
            {paginatedConsumptions.map((consumption) => (
              <div key={consumption.id} className="group relative bg-white border border-gray-200 rounded-xl p-6 hover:shadow-lg hover:border-gray-300 transition-all duration-200">
                {/* 主要信息区域 */}
                      ...
                      <div className="text-xs">
                        <div className="font-medium text-gray-900">
                          {consumption.consumptionAt 
                            ? dayjs(consumption.consumptionAt).format('YYYY-MM-DD')
                    ...
                    {consumption.rechargeInfo ? (
                      <div className="space-y-1">
                        ...
                        <div className="text-xs text-gray-600">
                          剩余 {consumption.rechargeInfo.remainingTimes}/{consumption.rechargeInfo.totalTimes} 次
                        </div>
                      </div>
                    ) : (
      ...
      {filteredConsumptions.length === 0 && (
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <h3 className="mt-4 text-lg font-medium text-gray-900">暂无消费记录</h3>
